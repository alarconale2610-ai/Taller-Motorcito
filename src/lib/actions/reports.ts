...
import type { Sale } from '@/types/database'; // Importar el tipo
...
export async function getSalesReport(branchId: string, days: number = 7) {
  ...
  // ✅ CORREGIDO: Agregar tipos explícitos
  type DayData = { amount: number; transactions: number };
  type SalesByDayItem = { date: string; amount: number; transactions: number };
  
  const salesByDay: SalesByDayItem[] = [];
  const groupedByDay = new Map<string, DayData>();

  sales?.forEach((sale: Sale) => { // ✅ Tipar sale explícitamente
    ...
    const current = groupedByDay.get(date)!; // ! indica que no es undefined
